<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="名额" prop="minge">
          <el-select :disabled="ro.minge" v-model="ruleForm.minge" placeholder="请选择名额">
            <el-option
                v-for="(item,index) in mingeOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="名额" prop="minge">
	      <el-input v-model="ruleForm.minge"
                placeholder="名额" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'&& !ro.chengjiziliao" label="成绩资料" prop="chengjiziliao">
          <file-upload
          tip="点击上传成绩资料"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.chengjiziliao?ruleForm.chengjiziliao:''"
          @change="chengjiziliaoUploadChange"
          ></file-upload>
        </el-form-item>  
        <div v-else>
          <el-form-item v-if="ruleForm.chengjiziliao" label="成绩资料" prop="chengjiziliao">
            <el-button type="text" size="small" @click="download($base.url+ruleForm.chengjiziliao)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.baomingziliao = true;
          ...
          if(o=='chengjiziliao'){
            this.ruleForm.chengjiziliao = obj[o];
	    this.ro.chengjiziliao = true;
            continue;
          }
          if(o=='baomingshijian'){
      ...
      this.$http({
		...
		if(json.shouji!=''&&json.shouji){
          ...
          this.$message.error(data.msg);
            ...
            this.mingeOptions = "1".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `kaoshengbaoming/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	if(this.ruleForm.chengjiziliao!=null) {
		this.ruleForm.chengjiziliao = this.ruleForm.chengjiziliao.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
      ...
      if(objcross!=null) {
	      objcross.minge = objcross.minge - this.ruleForm.minge
	      if(objcross.minge<0){
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `kaoshengbaoming/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.kaoshengbaomingCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    chengjiziliaoUploadChange(fileUrls) {
	this.ruleForm.chengjiziliao = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
